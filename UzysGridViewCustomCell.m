...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.image = [[UIImageView alloc]initWithFrame:CGRectNull];
        self.image.clipsToBounds = YES;
        //self.image.layer.cornerRadius = 5.0f;
        [self addSubview:self.image];
        [self bringSubviewToFront:self.ButtonDelete1];
...
- (void)layoutSubviews
{
    ...
    self.image.frame =CGRectInset(f, inset, 0);
    NSLog(@"selff---- %f------ %f",self.image.frame.size.width,self.image.frame.size.height);
